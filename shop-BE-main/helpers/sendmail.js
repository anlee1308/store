...
async function VerifyEmail(emailUser, url) {
  ...
  let info = await transporter.sendMail({
    from: '"exapmle 👻" <nhoxbuonlv3@example.com>', // sender address
    to: emailUser,
    ...
    html: `
            ...
            <h2 style="text-align: center; text-transform: uppercase;color: teal;">CHÀO MỪNG BẠN ĐẾN VỚI Book Store</h2>
            <p>Xin chúc mừng! Bạn sắp bắt đầu sử dụng Laptop . Chỉ cần nhấp vào nút bên dưới để xác thực địa chỉ email của bạn.</p>
            <a href=${url} target="_blank style="background: crimson; text-decoration: none; color: white; padding: 10px 20px; margin: 10px auto; display: block; border-radius: 20px; width: 260px; text-align: center;">Verify</a>
            </div>
        `, // html bod
  });
}
async function ChangePassword(emailUser, url) {
  ...
  let info = await transporter.sendMail({
    ...
    html: `
            <div style="max-width: 700px; margin:40px auto; border: 1px solid #ddd; padding: 50px 20px; font-size: 110%; border-radius: 10px;">
            <h2 style="text-align: center; text-transform: uppercase;color: teal;">THAY ĐỔI MẬT KHẨU</h2>
