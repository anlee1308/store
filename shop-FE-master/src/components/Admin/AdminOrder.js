  ...
  return (
    ListOrder && (
      <div className="order-wrap mt-4">
        <div className="order-wrap-item">{renderOrderItem(ListOrder)}</div>
        <div className="order-wrap-control">
          <div className="order-wrap-totalPrice">
            <span className="order-wrap-totalPrice-title">Tổng Số Tiền</span>
  ...
  function renderOrder(data, key) {
    ...
    if (data && data.data != "" && key == 1) {
      temp = data.map((item, stt) => (
        <AdminOrderItem key={stt} ListOrder={item} isRender={isRender} />
      ));
